...
" ,lcb
" c.get('value').toggle()
" save last project / save last project-buffer / repoen on start
" NewInstance (remove buffers, and everyghing)
"
" python << EOF
...
function! F(...)
  ...
  " redraw!
  "cexpr map(glob(ProjectPath()..'/**', 0, 1), '{"filename": v:val}')
...
function! FillSpaces(value, length)
  let start = len(string(a:value))
  let spaces=""
  for i in range(start, a:length)
    let spaces.=" "
  endfor
  return spaces
endfunction
...
"   if c.get('bufnr')==-1
"     " let buf=term
"     let c.get('bufnr')=GetBufDirectionIfTerm(c.get('direction'))
"     if c.get('bufnr')==-1
"       let [c.get('direction'),c.get('bufnr')]=FindSomeTerm()
"     endif
"     if c.get('bufnr')==-1
"       call Open('J', "terminal", "new")
"       let [c.get('direction'),c.get('bufnr')]=FindSomeTerm()
"     endif
"   elseif !bufexists(c.get('bufnr'))
"     call Open('J', "terminal", "new")
"     let [c.get('direction'),c.get('bufnr')]=FindSomeTerm()
...
let SaveIn = ["vim_configuration", "repo", "samedir", "infile"]
...
" extend (global/repo/folder/buffer)
  ...
  " (1) load them anyways when in in_vim_configuration / sometimes, when in projectroot / samedir or infile
...
function! FindCommand(cs, keymap=g:keymap)
  ...
  let counter=w:commandselections[a:keymap]
  if len(a:cs)>0 && counter<len(a:cs)
    let c=a:cs[counter]
    if !empty(c)
      return c
    else
...
"       call DebugBuf("3:  spectrum:           "..c['extend'])
...
"         call c.set('direction', 'h')
"       elseif index([106, 74], char)>-1
"         call c.set('direction', 'j')
"       elseif index([107, 75], char)>-1
"         call c.set('direction', 'k')
"       elseif index([108, 76], char)>-1
"         call c.set('direction', 'l')
...
function! Keypress_Handler() range
  ...
  " call DebugBuf(c)
  " let __x=g:cmdstorage.get('commands')
  " call DebugBuf(len(__x)>0?__x[0]:'')
  ...
  if g:mode=='visual'
    ...
    call RedefineOrCreateNew(keymap)
  elseif g:keymap=~",,"
    call DebugBuf(g:keymap)
    if !empty(c)
      call DebugBuf("BEFORE configure c = storage[0] = " . (c is g:cmdstorage.get("commands")[0]))
      call c.configure()
      call DebugBuf("AFTER  configure c = storage[0] = " . (c is g:cmdstorage.get("commands")[0]))
      call DebugBuf("extend after configure          = " . (c.get("extend")))
  ...
  elseif g:keymap=~","
    ...
    " call DebugBuf(c)
    if !empty(c)
      let b:savein=c.get('save')
      let b:released=c.get('released')
      let b:spectrum=c.get('extend')
      " call DebugBuf(c)
    else
...
let gc=TermCommand()
call gc.set('command', 'echo "TEST"')
call _command(gc)

let gc=TermCommand()
call gc.set('name', 'Simple Command')
call gc.set('command', 'echo "TEST"')
call _command(gc)
